...
def detect_objects(image, model, img_size=640):
    ...
    # Return detections for processing later
...
def gen_frames(n=20):  # n is the frame interval
    ...
    while True:
        ...
        if frame_counter % n == 0:
            detections, class_names = detect_objects(frame, model)
            ...
            # Update global variable or cache with the latest results
            app.detection_results = detection_results
...
def detection_results():
    results = getattr(app, 'detection_results', [])
